# Documentation index

Product and operator documentation for Shell Sentinel. Official site: <https://www.shellsentinel.net>.

> **Language policy:** project and collaboration docs are maintained in **English** only. Product and operator docs below must stay in sync across **English, Spanish, and German**.

## User guides (EN / ES / DE)

- [English](user_guide_en.md)
- [Spanish](user_guide_es.md)
- [German](user_guide_de.md)

## Dependencies (EN / ES / DE)

- [English](dependencies_en.md)
- [Spanish](dependencies.md)
- [German](dependencies_de.md)

## Technical guides

- [Custom model providers](custom_model_providers_es.md) — Spanish only today; add EN/DE when extended.

## Spanish operator index

Extended Spanish notes (including static-site SEO conventions) live in [README_es.md](README_es.md).

## Repository root

- [README.md](../README.md) — product overview (English, canonical on GitHub)
- [README_es.md](../README_es.md) · [README_de.md](../README_de.md) — translations
- [CHANGELOG.md](../CHANGELOG.md) · [CONTRIBUTING.md](../CONTRIBUTING.md) · [AGENTS.md](../AGENTS.md)
